...
<div class="mt-5">
    <x-section-card title="En attente de reprogrammation par le patient">
        @forelse ($aReprogrammer as $rdv)
            <div class="flex items-center justify-between px-5 py-3.5 {{ !$loop->last ? 'border-b border-slate-100' : '' }}">
                <div>
                    <p class="text-[13px] font-medium text-slate-900">{{ $rdv->patient->user->name }}</p>
                    <p class="text-[12px] text-slate-400 mt-0.5">Refusé le {{ $rdv->updated_at->format('d/m/Y à H:i') }} · en attente d'un nouveau créneau</p>
                </div>
                <span class="text-[11px] font-medium px-2 py-0.5 rounded-full bg-amber-50 text-amber-700 shrink-0">À reprogrammer</span>
            </div>
        @empty
            <div class="px-5 py-10 text-center"><p class="text-[13px] text-slate-400">Aucun rendez-vous en attente de reprogrammation.</p></div>
        @endforelse
    </x-section-card>
</div>
