...
@section('sidebar')
    @include('partials.sidebar-superadmin', ['active' => 'stats'])
@endsection
...
<script>
    ...
    new Chart(document.getElementById('semaineChart'), {
        type: 'line',
        data: { labels: semaineData.map(d => d.label), datasets: [{ label: 'Consultations', data: semaineData.map(d => d.count), borderColor: '#0B3A66', backgroundColor: 'rgba(11,58,102,0.08)', fill: true, tension: 0.35 }] },
        options: { plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } } }
    });
    const specData = @json($parSpecialite);
    new Chart(document.getElementById('specialiteChart'), {
        type: 'bar',
        data: { labels: specData.map(d => d.specialite), datasets: [{ label: 'Consultations', data: specData.map(d => d.total), backgroundColor: '#1E9E6B', borderRadius: 6 }] },
        options: { plugins: { legend: { display: false } }, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } } }
    });
</script>
@endpush